...
var functionWrapperTemplate = babelTemplate.template('$runtime$.async(function $id_$($params$) { $body$ }, $index$, $arity$)');
...
var requireVarTemplate = babelTemplate.template("var $name = typeof require === 'function' ? require($path) : Streamline.require($path);");
var requireVarMemberTemplate = babelTemplate.template("var $name = (typeof require === 'function' ? require($path) : Streamline.require($path)).$member;");
...
var programWrapperTemplate = babelTemplate.template("$runtime$.async($arg$, 0, 1).call(this, function(err) { if (err) throw err; })");
...
var varDeclTemplate = babelTemplate.template("var $name$ = $value$;");
