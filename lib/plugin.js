...
var functionWrapperTemplate = template('$runtime$.async(function $id_$($params$) { $body$ }, $index$, $arity$)');
...
var requireVarTemplate = template("var $name = typeof require === 'function' ? require($path) : Streamline.require($path);");
var requireVarMemberTemplate = template("var $name = (typeof require === 'function' ? require($path) : Streamline.require($path)).$member;");
...
var programWrapperTemplate = template("$runtime$.async($arg$, 0, 1).call(this, function(err) { if (err) throw err; })");
...
var varDeclTemplate = template("var $name$ = $value$;");
