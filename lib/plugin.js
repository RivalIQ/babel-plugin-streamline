...
var functionWrapperTemplate = babelTemplate('$runtime$.async(function $id_$($params$) { $body$ }, $index$, $arity$)');
...
var requireVarTemplate = babelTemplate("var $name = typeof require === 'function' ? require($path) : Streamline.require($path);");
var requireVarMemberTemplate = babelTemplate("var $name = (typeof require === 'function' ? require($path) : Streamline.require($path)).$member;");
...
var programWrapperTemplate = babelTemplate("$runtime$.async($arg$, 0, 1).call(this, function(err) { if (err) throw err; })");
...
var varDeclTemplate = babelTemplate("var $name$ = $value$;");
